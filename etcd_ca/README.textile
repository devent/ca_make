...
Contains Make script to create certificates for a etcd cluster. Specifically, it creates a certificate authority to self sign the certificates, a etcd server certificate and client certificates.
...
* The project is based on the Github project that can be found on "kelseyhightower/etcd-production-setup [github.com]":https://github.com/kelseyhightower/etcd-production-setup
...
The @openssl_server.cnf@ and @openssl_worker.cnf@ file contains the configurations for the etcd server and workers, respectively, of the Openssl client. The top of the file contains some default values to create server certificates and should be changed to one's own needs.

* @ETCD_SAN@, the IP address(es) of the etcd servers address(es);
...
Creates a new etcd server certificate, needed arguments are @ETCD_SAN@ and @ETCD_NAME@.

* @make client@

Creates a new etcd client certificate.
...
<pre>
make
make server ETCD_SAN="IP:127.0.0.1, IP:10.0.1.10" ETCD_NAME=k8-master.muellerpublic.de
make client
</pre>
